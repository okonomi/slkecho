...
RSpec.describe Slkecho::OptionParser do
  describe "#parse_options" do
    ...
    context "when no arguments" do
      ...
      it { is_expected.to have_attributes(channel: nil, subject: nil, message: nil) }
    end

    context "when channel option is given" do
      let(:argv) { %w[-c #general] }

      it { is_expected.to have_attributes(channel: "#general", subject: nil, message: nil) }
    end

    context "when channel and subject option is given" do
      let(:argv) { %w[-c #general -s subject] }

      it { is_expected.to have_attributes(channel: "#general", subject: "subject", message: nil) }
    end

    context "when channel and subject and message option is given" do
      let(:argv) { %w[-c #general -s subject message] }

      it { is_expected.to have_attributes(channel: "#general", subject: "subject", message: "message") }
    end
  end
